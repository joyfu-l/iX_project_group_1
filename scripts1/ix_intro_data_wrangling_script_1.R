...
# what does `dodge` do?
#'dodge' seperates the bars instead of having them overlap each other

# A different approach

ggplot(data = df) + 
  geom_density(mapping = aes(x = age_at_survey, fill = gender), alpha = 0.3)
#age v.s. age_at_survey
# NAs and older people cloud our view
# alpha = X gives us a trasparent color fill so we can see the different ones
# tidyverse::filter
...
df_eh <- df %>% 
  mutate(work_experience = if_else(leadershiprole == "Yes" & working == TRUE, 1, 0))
